...
function main()
{
    ...
    {
        tex.image.onload = () => callback(null, tex);
        tex.image.onerror = () => callback(new Error(`Error while loading image "${src}"`));
        tex.image.src = src;
    }
    ...
    load_image(textures[0], '../resources/sky.jpg', 
        function(err, tex0) {
            if(err)
            {
                console.log(err.message + ' for texture[0]');
            }
            else
            {
                init_texture(gl, tex0);
                load_image(textures[1], '../resources/circle.gif',
                    function(err, tex1) {
                        if(err)
                        {
                            console.log(err.message + ' for texture[1]');
                        }
                        else
                        {
                            init_texture(gl, tex1);
                            tick();
                        }
                    }
                );
