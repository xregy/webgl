...
function main()
{
    ...
    const V = mat4.create();
    mat4.lookAt(V, [3, 3, 3], [0, 0, 0], [0, 1, 0]);
    ...
    list_shaders["Blinn-Gouraud"] = new Shader(gl, 
        shaders.src_vert_Blinn_Gouraud_spot({loc_aPosition, loc_aNormal, numLights}),
        shaders.src_frag_Blinn_Gouraud_spot(), uniform_vars);
    list_shaders["Phong-Gouraud"] = new Shader(gl, 
        shaders.src_vert_Phong_Gouraud_spot({loc_aPosition, loc_aNormal, numLights}), 
        shaders.src_frag_Phong_Gouraud_spot(), uniform_vars);
    list_shaders["Blinn-Phong"] = new Shader(gl, 
        shaders.src_vert_Blinn_Phong_spot({loc_aPosition, loc_aNormal}),
        shaders.src_frag_Blinn_Phong_spot({numLights}), uniform_vars);
    list_shaders["Phong-Phong"] = new Shader(gl, 
        shaders.src_vert_Phong_Phong_spot({loc_aPosition, loc_aNormal}),
        shaders.src_frag_Phong_Phong_spot({numLights}), uniform_vars);
    ...
    function tick()
    {
        ...
        mat4.rotate(light.M, light.M, toRadian(( (ANGLE_STEP_LIGHT * elapsed) / 1000.0) % 360.0), [0, 1, 0]);
        for(let i=0 ; i<3 ; i++)
            light.direction[i] = -light.position[i];
