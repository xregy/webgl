...
//			'}\n';
		...
		if(!this.shader_pick)
			this.shader_pick = new Shader(gl, src_shader_pick_vert, src_shader_pick_frag, ['aPosition']);
	}
	...
	render_id(gl, V, P)
	{
		var	h_prog = this.shader_pick.h_prog;
		...
		for(var attrib_name in this.shader_pick.attribs)
		{
			var attrib = this.attribs[attrib_name];
			gl.bindBuffer(gl.ARRAY_BUFFER, attrib.buffer);
			gl.vertexAttribPointer(this.shader_pick.attribs[attrib_name], attrib.size, attrib.type, 
				attrib.normalized, attrib.stride, attrib.offset);
			gl.bindBuffer(gl.ARRAY_BUFFER, null);
			gl.enableVertexAttribArray(this.shader_pick.attribs[attrib_name]);
		}
		if(this.draw_call == "drawArrays")
		{
			gl.drawArrays(this.draw_mode, 0, this.n);
		...
		for(var attrib_name in this.shader_pick.attribs)
		{
			gl.disableVertexAttribArray(this.shader_pick.attribs[attrib_name]);
		}
		gl.useProgram(null);
